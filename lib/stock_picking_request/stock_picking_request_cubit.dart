...
class StockPickingRequestCubit extends Cubit<StockPickingRequestState> {
  ...
  Future<void> updateProductQuantity(
      int pickingId, int moveId, double quantityDone) async {
    try {
      // Try to update using reserved_availability field instead of quantity_done
      // This is a common field in Odoo that serves a similar purpose
      final success = await odooService.updateRecord(
        'stock.move',
        moveId,
        {'product_uom_qty': quantityDone}, // Update the demand quantity instead
      );

      if (success) {
        await fetchDetail(pickingId);
      } else {
        emit(StockPickingRequestDetailError('Failed to update quantity'));
      }
    } catch (e) {
      emit(StockPickingRequestDetailError('Error updating quantity: $e'));
    }
  }

  Future<bool> removeProductLine(int pickingId, int moveId) async {
    try {
      // First check if the move can be deleted
      final moveData = await odooService.fetchRecords(
        'stock.move',
        [
          ['id', '=', moveId]
        ],
      ...
      if (moveData.isEmpty) {
        emit(StockPickingRequestDetailError('Move not found'));
        return false;
      }

      final moveState = moveData.first['state'];
      if (moveState == 'done' || moveState == 'cancel') {
        emit(StockPickingRequestDetailError('Cannot remove a processed move'));
        return false;
      }
      ...
      if (result == true) {
        ...
        emit(StockPickingRequestDetailError('Failed to remove product line'));
        return false;
      }
    } catch (e) {
      emit(StockPickingRequestDetailError('Error removing product line: $e'));
      return false;
    }
  }
}
