            ...
            Row(
              ...
              children: [
                ...
                Container(
                  ...
                  child: Text(
                    _getStatusname(
                        context, order['state']), // Pass context here
                    style: GoogleFonts.poppins(
                      fontSize: 12,
...
String _getStatusname(BuildContext context, String state) {
  switch (state) {
    case 'draft':
...
void updateStatus(BuildContext context, int id, String status) {
  DeliveryOrderCubit.get(context).updateDeliveryStatus(id, status);
}
...
void filterByStatus(BuildContext context, String status) {
  final cubit = DeliveryOrderCubit.get(context);
  if (status == 'all') {
    cubit.fetchDeliveryOrders();
  } else {
